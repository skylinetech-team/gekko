...
export default {
  ...
  computed: {
    ...
    watchConfig: function() {
      const raw = _.pick(this.config, 'watch', 'candleWriter')
      const watchConfig = Vue.util.extend({}, raw)
    ...
    requiredHistoricalData: function() {
      ...
      const stratSettings = this.config.tradingAdvisor
    ...
    gekkoConfig: function() {
      ...
      if (!this.requiredHistoricalData) {
        ...
        const available = window.moment.utc(this.existingMarketWatcher.events.initial.candle.start).unix()

        startAt = window.moment
